...
3. The third part is, in a sense, the recursion itself and what might also be called a **recrusive member**. When calling the CTE for the first time (R<sub>1</sub>) we're returning the iteration before the union, here `1`. We select the value and add a 1 to it, returning for the second iteration (R<sub>2</sub>) the value 2. This repeats for the third, fourth (R<sub>n</sub>)... until we reach a **termination condition**, a threshold we set, here `WHERE N < 10`, terminating our recursion. 
...
If we were to run the above query and try to produce 1000 numbers we'd get an error since we'd exceed the default of 100 iterations[^1]. You can see something similar - and more explicit - with the following code chunk:


[^1]: Though for some reason here it doesn't throw an error so I had to give an example with an explicit lower threshold.
...
```
## Error in result_fetch(res@ptr, n): nanodbc/nanodbc.cpp:2695: 23000: [Microsoft][ODBC SQL Server Driver][SQL Server]The statement terminated. The maximum recursion 5 has been exhausted before statement completion.
```

We set the number of iterations to be 5, but our recursion's termination condition is at 10. Once we reach 5 iterations we receive an error that we exhausted the max recursion and our whole process breaks.

If you want your recursion to go more than the default, just set the `MAXRECURSION OPTION` to what you need. For an unlimited option use the value 0:
