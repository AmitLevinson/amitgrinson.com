...
SELECT u.user_id,
  ...
  rnk = DENSE_RANK() OVER(PARTITION BY COUNTRY ORDER BY sum(amount) DESC)
FROM USERS U 
LEFT JOIN PAYMENTS P ON P.USER_ID = u.user_id
GROUP BY u.user_id, u.country
)
...
The key part here is the `rnk` column we create which I belive also highlights the order of operations in our query: We use the aggregate value of the group by, `SUM(Amount)`, within our new column that's a window function.

In terms of order of operations they are as follows:
...
Window functions are called in the `SELECT` statement and thus can be called *after* we aggregated. So in this case, we order our window function as a function of the aggregate sum we called per user. We then filter to the user with highest amount in the outer query.
