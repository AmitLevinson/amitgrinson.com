...
```{r eval = FALSE}
window_function <- dbGetQuery(localhost_con, "SELECT UserName, Payment_Date as 'Payment Date', amount
FROM (
  SELECT *,
    DENSE_RANK() OVER(Partition BY UserName Order by amount DESC) as rnk
  FROM payments) AS ranked_table
WHERE rnk = 1")
microbenchmark::microbenchmark(dbGetQuery(localhost_con, "SELECT UserName, Payment_Date as 'Payment Date', amount
FROM (
  SELECT *,
    DENSE_RANK() OVER(Partition BY UserName Order by amount DESC) as rnk
  FROM payments) AS ranked_table
...
dbGetQuery(localhost_con, "SELECT DISTINCT p.UserName, p.payment_date, p.amount
FROM payments p
LEFT JOIN payments pp ON p.UserName = pp.UserName
  AND p.amount < pp.amount
WHERE pp.amount IS NULL")
...
eval_query <- function (x) {
  
 (dbGetQuery(localhost_con, x))
}

window_function <- expression((dbGetQuery(localhost_con,"SELECT UserName, Payment_Date as 'Payment Date', amount
FROM (
  SELECT *,
    DENSE_RANK() OVER(Partition BY UserName Order by amount DESC) as rnk
  FROM payments) AS ranked_table
...
join_clause <- expression((dbGetQuery(localhost_con,"SELECT DISTINCT p.UserName, p.payment_date, p.amount
FROM payments p
LEFT JOIN payments pp ON p.UserName = pp.UserName
  AND p.amount < pp.amount
WHERE pp.amount IS NULL")))

microbenchmark::microbenchmark(window_function, join_clause)
```
