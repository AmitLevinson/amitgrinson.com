...
Note that I'll be using R to setup an SQL connection in memory but power through the blog post with SQL. While not necessary, some SQL knowledge is useful to understanding the various answers' syntax.
...
If your new to SQL, an immediate question that might come to mind is why not use a `GROUP BY` for the UserName, date and select the `MAX` value. I.e., just filter each observation by the max value according to one of the variables.  
...
A total of 2,000 rows for all the three users. Since we'll be benchmarking using R, let's write a function to evaluate the SQL queries and run it across all three approaches:
...
benchmarking <- microbenchmark::microbenchmark("Window" = runquery(Window_script),
                                               "Join" = runquery(Join_script),
...
We can see that for the current question, the window function is most efficient. One ceaveat is that maybe a slowdown occured when I queried the local database through R on my local machine, but if so I'd imagine it would affect all options.

The window function is best here, but I think knowing all approaches can help you write better SQL. That is, sometimes one approach is a better fit to a specific use case. I definitely wrote a correlated subquery at work as it was the best fit at the time (in terms of readability and as an immediate answer), so though it's least efficient here it's worth knowing.
