...
df_char <- df_survey %>% 
  ...
  select(-c(status:q1, q9_1:q10_5,q18_1:q29_5))
```
...
**4. Sum of the responses aren't ordered** - Notice how some of the responses are randomly ordered, when ideally we'd want them to be ordered by hierarchy. For example question XXXXX
...
df_q_clean <- df_questions %>% 
  clean_names() %>% 
  pivot_longer(cols = q2:q38, names_to = "question", values_to = "text") 
...
In our first `str_remove_all` we remove all letters
, but let's assume we have some more details about our questions, such as the wording itself. For exmaple, when downloading Qualtrics survey data, you also get the wording of the question. We can use that to provide more information about our data:

```{r}
# Merge the table with the questions
# Run the kable again
```

Great, so we have our actual questions, but now it's a littlle messey considering it's duplicated in each row. Here we can use the amazing `collapse_rows()` function which, as the name suggests, collapses the rows of a column we choose. Let's see it in action:
