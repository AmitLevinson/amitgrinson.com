---
title: Regex and Text operations in Python & R Vs. SQL
...
A faster approach instead of the `apply` would be to filter using list comprehension but I find it much less readable, as well as making sure no `NaN` are in place (which makes sense given were cross joining). [^2]

[^2]: It's actuallly XX times faster (timed on YY rows). We could achieve it using: `.loc[lambda df: [x[0] in x[1] if x[0] is not None else False for x in zip(df['id'], df['payment_identifier'])]`
...
Besides the basics you probably know of if you've work with SQL, there were a few other cases I was glad to learn something new when working with text in SQL. Here are a few last tips I have for you before you go:

1. Though not shown above, you can use regex-like operations, for example you can use in the where clause a filtering to identify a string that is a letter, number, then letter without needing to replace using the following `...Like [a-z][0-9][a-z]. 

2. Morever you can also use case sensitive operations to match lower and uppercase when needed, just add `WHERE column COLLATE Latin1_General_BIN LIKE [A-Z]...` to have it case sensitive.
