...
```{python, echo = FALSE}
def id_isin_identifier(payment_identifier, id):
  if id in payment_identifier:
    return True
  else:
    return False
```
...
```{python, eval = FALSE}
(payments_joined.loc[:, ['payment_identifier', 'id']]
 .pipe(lambda df: df[df.apply(lambda x: id_isin_identifier(x['payment_identifier'], x['id']), axis = 1)])
)
```
