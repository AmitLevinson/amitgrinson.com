...
Working as a Data Analyst in the fraud domain I constantly find myself manipulating text, mainly for things such as abstracting a pattern we identified. This is done with various text manipulations in SQL or locally in Python or R using regex. 
...
Awesome, we setup a connection to our local server on the computer and can write explicit SQL queries. As we move forward we'll be working with both Python (mainly) and a little with R. For that let's load the relevant libraries and data:
...
these strings are contained in some of our payment_identifiers, but how can we easily join them considering it's not an exact match? We'll solve it using a join - and not filtering by the pattern - so we can match each identifier to the payment returned.
