...
**TL;DR**: In this post we setup the code for having a GitHub Action automatically update a plot. I start by showing how to plot my most frequently used R packages for Tidytuesday. However, I constantly add more information to that data and need to re-run the script to update the plot. For that purpose we turn to GitHub Actions that provide the ability to run an R script on a specific commit. We setup the script for the GitHub Action to run our plot and review the various arguments.
...
As to Michael, I thank him for the example I use in the post to try out GitHub Actions. A while back Michael posted in an Israeli R Facebook group a plot detailing the packages he frequently uses ([here's a gist with the script](https://gist.github.com/michaeldorman/ad8d89136f03769105ccc6199a913f0b?fbclid=IwAR09u1WFD9YpKYb7nvezOKqGGZac3AuVWC7ogDItSv9sXXS0x2KpYxg3aU0) ). When thinking of how to try out GitHub actions I thought replicating it would be a great example, only adding an automation as a twist. 
...
I first heard about GitHub Actions in regards to package checks, enabling package developers to run continuous integrations, unit tests and the like automatically. **You can have GitHub run a script when a Git action occurs (e.g. a push or pull request), or at a specific time slot in the day/week/month.** 
