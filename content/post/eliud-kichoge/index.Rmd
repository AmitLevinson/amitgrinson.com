...
knitr::include_graphics("/img/marath_record/new_record.jpg")
```
...
knitr::include_graphics("/img/marath_record/wikipage.png")
```

Here we can see that the webpage contains information about Marathon records, where in the screenshot we see the Men section. We only want this table with Men's records, so let's get that:
...
Let's see what we did here. First I add Eliud Kipchoge's new unofficial record as an observation into our dataframe. I then turned to the `{lubridate}` package where I used the `hms` function to mutate the time variable we had into an new variable called 'r_period_raw'. Although this cleans the variable, `hms` transforms it into a a `period` object which I found a little difficult to use when we want to plot. What we need is to turn it into a numeric class which we did in our new variable 'run_duration'. This will help us in plotting but I retained the period class variable as it makes it easier to read in this case.  

I then turned the date column into a Month-Day-Year variable using the `mdy` function, which eventually I only extracted the year using `year`. Lastly I discarded the old columns we don't need anymore. We also recieved a warning sign that one observation didn't parse. This was because the value in the cell didn't match the pattern of the `hms` fuction. The original pattern looked like this: `r runners_clean$date[runners_clean$name == 'Henry Barrett']`. All we want is the specific year so it's no big deal, but anyway let's go ahead and manually add it if we decide to use it later:
